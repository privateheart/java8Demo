...
public class TradeDemo {
    ...
    public static void main(String[] args) {
        ...
        transactions.stream().filter( transaction -> {return transaction.getYear() == 2011;}).sorted(comparing(Transaction::getValue)).forEach(t ->{
            System.out.println(t);
        });
        //(2) 交易员都在哪些不同的城市工作过
        System.out.println("(2) 交易员都在哪些不同的城市工作过");
        transactions.stream().map( t -> {return t.getTrader().getCity();}).distinct().forEach(city -> System.out.println(city));
        //(3) 查找所有来自于剑桥的交易员，并按照姓名排序
        System.out.println("(3) 查找所有来自于剑桥的交易员，并按照姓名排序");
        transactions.stream().filter(t -> {return "Cambridge".equals(t.getTrader().getCity());}).map( t -> {return t.getTrader();}).sorted(comparing(Trader::getName)).forEach(t -> System.out.println(t));
        //(4) 返回所有交易员的姓名字符串，按字母顺序排序
        System.out.println("(4) 返回所有交易员的姓名字符串，按字母顺序排序");
        transactions.stream().map(t -> {return t.getTrader().getName();}).distinct().sorted(String::compareTo).forEach(n -> {
        ...
        transactions.stream().filter(t-> {return "Cambridge".equals(t.getTrader().getCity());});
        //(7) 所有交易中，最高的交易额是多少？
